...
async def del_sudo_user(client, message):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.edit("ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴜsᴇʀ's ᴍᴇssᴀɢᴇ ᴏʀ ɢɪᴠᴇ ᴜsᴇʀɴᴀᴍᴇ/ᴜsᴇʀ_ɪᴅ.")
        user = message.text.split(None, 1)[1]
        if "@" in user:
            user = user.replace("@", "")
        user = await app.get_users(user)
        if user.id not in SUDOERS:
            return await message.edit("ɴᴏᴛ ᴀ ᴘᴀʀᴛ ᴏғ ʙᴏᴛ sᴜᴅᴏ.")
        removed = await del_sudo(user.id)
        if removed:
            SUDOERS.remove(user.id)
            await message.edit("ʀᴇᴍᴏᴠᴇᴅ ғʀᴏᴍ ʙᴏᴛ sᴜᴅᴏ ᴜsᴇʀ")
            return
        else:
    ...
    if user_id not in SUDOERS:
        return await message.edit("ɴᴏᴛ ᴀ ᴘᴀʀᴛ ᴏғ ʙᴏᴛ Suʏdo.")
