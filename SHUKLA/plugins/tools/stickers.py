    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await um.edit("**❍ ᴛʜᴇ sᴛɪᴄᴋᴇʀ ʜᴀs ɴᴏ ɴᴀᴍᴇ..!**")
                return
            ...
            await um.edit("**❍ ᴜɴsᴜᴘᴘᴏʀᴛᴇᴅ ғɪʟᴇs**")
            return
        media_ = await client.download_media(replied, file_name="ProjectMan/resources/")
    else:
        await um.edit("**❍ ᴘʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ ᴘʜᴏᴛᴏ/ɢɪғ/sᴛɪᴄᴋᴇʀ ᴍᴇᴅɪᴀ..!**")
        return
    if media_:
        ...
        while True:
            ...
            if exist.set.count >= limit:
                ...
                await um.edit(
                    f"`❍ ᴄʀᴇᴀᴛᴇ ᴀ ɴᴇᴡ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ {pack} ʙᴇᴄᴀᴜsᴇ ᴛʜᴇ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ ɪs ғᴜʟʟ`"
                )
                continue
            break
        if exist is not False:
            ...
            while limit in await get_response(message, client):
                ...
                if is_video:
                    ...
                    packnick += " (Video)"
                await um.edit(
                    "`❍ ᴄʀᴇᴀᴛᴇ ᴀ ɴᴇᴡ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ "
                    + str(pack)
                    + "❍ ʙᴇᴄᴀᴜsᴇ ᴛʜᴇ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ ɪs ғᴜʟʟ`"
                )
                await client.send_message("stickers", packname)
                await asyncio.sleep(2)
            ...
            if (
                ...
                == "❍ sᴏʀʀʏ, ᴛʜᴇ ғɪʟᴇ ᴛʏᴘᴇ ɪs ɪɴᴠᴀʟɪᴅ."
            ):
                await um.edit(
...
async def packinfo(client: Client, message: Message):
    rep = await message.edit_text("`❍ ᴘʀᴏᴄᴇssɪɴɢ...`")
    if not message.reply_to_message:
        await rep.edit("❍ ᴘʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ sᴛɪᴄᴋᴇʀ...")
        return
    if not message.reply_to_message.sticker:
        await rep.edit("❍ ᴘʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ ᴀ sᴛɪᴄᴋᴇʀ...")
        return
    if not message.reply_to_message.sticker.set_name:
        await rep.edit("`❍ sᴇᴇᴍs ʟɪᴋᴇ ᴀ sᴛʀᴀʏ sᴛɪᴄᴋᴇʀ!`")
        return
...
async def cb_sticker(client: Client, message: Message):
    ...
    if not query:
        return await message.edit_text("**❍ ᴇɴᴛᴇʀ ᴛʜᴇ ɴᴀᴍᴇ ᴏғ ᴛʜᴇ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ..!**")
    xx = await message.edit_text(message, "`❍ sᴇᴀʀᴄʜɪɴɢ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋs...`")
    text = requests.get(f"https://combot.org/telegram/stickers?q={query}").text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await xx.edit("**❍ ᴄᴀɴɴᴏᴛ ғɪɴᴅ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ**")
    reply = f"**❍ ᴋᴇʏᴡᴏʀᴅ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ:**\n {query}\n\n**❍ ʀᴇsᴜʟᴛs:**\n"
    for pack in results:
        if pack.button:
...
async def tinying(client: Client, message: Message):
    ...
    if not (reply and (reply.media)):
        return await message.edit_text("**❍ ᴘʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ sᴛɪᴄᴋᴇʀ ᴍᴇssᴀɢᴇ..!**")
...
async def memify(client: Client, message: Message):
    if not message.reply_to_message_id:
        await message.edit_text("**❍ ᴘʟᴢ ʀᴇᴘʟʏ ᴛᴏ ᴀɴ sᴛɪᴄᴋᴇʀ..!**")
        return
    reply_message = message.reply_to_message
    if not reply_message.media:
        await message.text("**❍ ᴘʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ ᴘʜᴏᴛᴏ ᴏʀ sᴛɪᴄᴋᴇʀ..!**")
        return
    file = await client.download_media(reply_message)
    mm = await message.edit_text("`❍ ᴘʀᴏᴄᴇssɪɴɢ . . .`")
    text = get_arg(message)
    if len(text) < 1:
        return await mm.edit(f"`❍ ᴘʟᴇᴀsᴇ ᴛʏᴘᴇ `.mmf text")
...
async def stick2png(client: Client, message: Message):
    try:
        await message.edit("`❍ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ . . .`")
        ...
        return await client.send_message(
            message.chat.id, f"**❍ ɪɴғᴏ:** `{e}`", reply_to_message_id=ReplyCheck(message)
        )

__NAME__ = "Sᴛɪᴄᴋᴇʀ"
__MENU__ = """
`.kang` - **ʀᴇᴘʟʏ .ᴋᴀɴɢ ᴛᴏ sᴛɪᴄᴋᴇʀ ᴏʀ ɪᴍᴀɢᴇ ᴛᴏ ᴀᴅᴅ ᴛᴏ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ**
`.packinfo` - **ᴛᴏ ɢᴇᴛ sᴛɪᴄᴋᴇʀ ᴘᴀᴄᴋ ɪɴғᴏʀᴍᴀᴛɪᴏɴ.**
`.mtoi` - **ʀᴇᴘʟʏ ᴛᴏ sᴛɪᴄᴋᴇʀ ᴛᴏ ɢᴇᴛ sᴛɪᴄᴋᴇʀ ᴘʜᴏᴛᴏ.**
`.mm` - **ʀᴇᴘʟʏ ᴛᴏ ᴍᴇssᴀɢᴇ sᴛɪᴄᴋᴇʀ ᴏʀ ᴘʜᴏᴛᴏ ᴡɪʟʟ ʙᴇ ᴄᴏɴᴠᴇʀᴛᴇᴅ ᴛᴏ ᴛʜᴇ sᴘᴇᴄɪғɪᴇᴅ ᴍᴇᴍᴇ ᴛᴇxᴛ sᴛɪᴄᴋᴇʀ.**
